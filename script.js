...
const dropAreaSelf = document.getElementById('dropAreaSelf');
const fileInputSelf = document.getElementById('fileInputSelf');
...
setupSelfUpload(dropAreaSelf, fileInputSelf);
...
function handleFile(file) {
  ...
  r.onload = e => {
    ...
    dropAreaSelf.style.display = 'block';
    buildDashboard(rows);
  };
  r.readAsText(file);
}
...
function handleSelfFile(file) {
  ...
  r.onload = e => {
    ...
    dropAreaSelf.style.display = 'none';
...
function parseCSV(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    obj.mutual = obj.mutual === 'True' || obj.mutual === 'true' || obj.mutual === '1';
    obj.profile_followers = parseInt(obj.profile_followers) || 0;
    obj.profile_following = parseInt(obj.profile_following) || 0;
    obj.profile_ratio = parseFloat(obj.profile_ratio) || 0;
    obj.days_active = parseInt(obj.days_active) || 0;
    rows.push(obj);
  }
  return rows;
}
...
function buildDashboard(data) {
  ...
  const selfRow = data.find(d => d.status === 'self');
  ...
  if (selfRow) {
    ...
    if (!globalSelfHistory) document.getElementById('selfUploadInline').style.display = 'block';
  ...
  new Chart(document.getElementById('selfFollowersLine'), {
    ...
    data: {
      ...
      datasets: [{
        ...
        fill: true,
        tension: 0.3,
        pointRadius: sorted.length > 30 ? 2 : 5,
        pointHoverRadius: 8,
...
function buildLine(id, sub, color, colorA) {
  ...
  const labels = sorted.map(d => {
    const dt = new Date(d.followed_at);
  ...
  new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        ...
        data: origins.map(o => parseFloat(o.avg.toFixed(3))),
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label: ctx => {
              const o = origins[ctx.dataIndex];
              return ` avg: ${o.avg.toFixed(3)} (n=${o.count})`;
            }
          }
        }
      }
...
function buildMaxRatioByOrigin(id, active) {
  ...
  new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        label: 'Ratio Máximo',
        data: origins.map(o => parseFloat(o.max.toFixed(3))),
...
function buildUqScatter(active) {
  ...
  const xMin = Math.max(0, Math.min(...all.map(p => p.x)) - 5);
  const xMax = Math.min(100, Math.max(...all.map(p => p.x)) + 5);
  ...
  new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        ...
        data: origins.map(o => parseFloat(o.avg.toFixed(1))),
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label: ctx => {
              const o = origins[ctx.dataIndex];
              return ` avg score: ${o.avg.toFixed(1)} (n=${o.count})`;
            }
          }
        }
      }
